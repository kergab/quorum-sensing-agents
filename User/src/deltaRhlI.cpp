...
deltaRhlI::deltaRhlI(deltaRhlIStrain* str) : Bacter(str) {}

void deltaRhlI::setState(Node* rhli_sign_node, Node* lasi_sign_node, Node* haas_node) {
...
unsigned deltaRhlI::step(Diffusion& diffusion) {
	...
	double RHLC = (di_rls_node->value > str()->rhlc_half_effect)? 1 : di_rls_node->value / str()->rhlc_half_effect;
	double RHLB = (mono_rls_node->value > str()->rhlb_half_effect)? 1 : mono_rls_node->value / str()->rhlb_half_effect;
	double RHLA = (haas_node->value > str()->rhla_half_effect)? 1 : haas_node->value / str()->rhla_half_effect;
	double RHLABC = (RHLC + RHLB + RHLA > 1)? 1 : (RHLC + RHLB + RHLA);
	
	v0 = v_max / str()->v_fraction;
	v = v0 + (v_max - v0)*RHLABC;
	QPointF gradSpeed = food_node->grad() * RHLC;
	...
	else if(mass > (str()->division)) return Divide;
	return Normal;
}
